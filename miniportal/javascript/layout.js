...
require(["vs/mpvlib", "vs/mpvapp"], function(mpvlib, mpvapp) {
    try {vslib.log('layout try vslib');} catch(ex) {vslib = new mpvlib();}
    vslib.log('vslib ready');
    vsapp = new mpvapp(); vslib.log('vsapp ready');
});

function log(str, doalert) {
    vslib.log(str, doalert);
};
...
function initMap(options) {
    ...
    dojox.layout.FloatingPane.prototype.show = function(callback) {
        var anim = dojo.fadeIn({node:this.domNode, duration:this.duration,
            beforeBegin: dojo.hitch(this,function(){
                ...
                if (this._dockNode) { this._dockNode.destroy(); this._dockNode = null; }
            })
        });
        try {
            anim.play(); this.resize(dojo.coords(this.domNode));
        } catch(ex) {vslib.log("dojox.layout.FloatingPane.prototype.show fail");}
    };
    ...
    configOptions.displayaddlayer = (configOptions.displayaddlayer === "true" || configOptions.displayaddlayer === true) ? true : false;
    configOptions.displaygetlink = (configOptions.displaygetlink === "true" || configOptions.displaygetlink === true) ? true : false;
...
function initUI(response) {
    dojo.connect(map, "onUpdateStart", function() {
        dojo.byId("logo").innerHTML = '<img src="images/algis_load.gif" />';
    });
    dojo.connect(map, "onUpdateEnd", function() {
        dojo.byId("logo").innerHTML = '<a href="http://www.allgis.org" target="_blank"><img src="images/algis_logo.png" /></a>';
    });
    ...
    if (displayLeftPanel()) {
        ...
        var cp = new dijit.layout.ContentPane({
            ...
            }, '<div style="float:right;clear:both;" id="paneCloseBtn"><a title=${close_title} alt=${close_alt} href="#" onclick="hideLeftPanel();"><img src=images/closepanel.png border="0"/></a></div>')
        });
        bc.addChild(cp);
    ...
    // Display 'add layer' button if enabled
    if (configOptions.displayaddlayer === true) {
        createAddLayerMenu();
    }
    ...
    // Display 'Get link to map' aka 'Save' button if enabled
    if (configOptions.displaygetlink === true) {
        createGetLinkButton();
    }
...
function addBasemapGalleryMenu() {
    ...
    vslib.log('addBasemapGalleryMenu');
    var ht = map.height / 2;

    var cp = dijit.byId('basemapGallery');
    if(cp) {;}
    else {
...
function addLayerList(layers) {
    ...
    if (layerList.length > 0) {
        ...
        var button = dijit.byId("layerBtn");
        if(button) {
...
function updatePrint(templates){
    ...
    // todo: need global loading marker (for map and print),
    // because loading indicator stoped if we zoom map in this moment
...
function addMeasurementWidget() {
    vslib.log("addMeasurementWidget");
    ...
    var fp = new dojox.layout.FloatingPane({
        ...
        dockable: false,
        closable: false,
        style: "position:absolute;top:0;left:50px;width:265px;height:165px;z-index:100;visibility:hidden;",
        id: 'floater'
    }, dojo.byId('floater'));
    fp.startup();
    ...
    vslib.log("addMeasurementWidget. div.id: " + closeDiv.id);
...
function toggleMeasure() {
    ...
    if (dojo.byId('floater').style.visibility === 'hidden') {
        ...
        if(!editorWidget){
            ...
            vslib.log('toggleMeasure. Editor widget exists so no disabling');
        }
    } else {
...
function createSocialLinks() {
    ...
    function createSearchTool() {
        ...
        var geocoder = dijit.byId('webmap-toolbar-right');
        if(geocoder) {;}
        else {
    ...
    function setupInfoWindowAndZoom(content, geocodeLocation, extent, pos){
        ...
        if(allResults.length > 1){
            ...
            vslib.log("setupInfoWindowAndZoom. attr: ");
            vslib.dir(attr);
...
function addTimeSlider(timeProperties) {
    ...
    vslib.log("addTimeSlider. div.id: " + closeDiv.id);
...
function createTimeSlider(timeProperties) {
    ...
    vslib.log("createTimeSlider. div.id: "+ timeSliderLabel.id);
...
function addToc(layerInfo) {
    ...
    dojo.connect(tocTb, 'onClick', function () {
        ...
        dijit.byId('legendAndTOC').refresh(true);
    });
...
// Create menu of addLayer options
function createAddLayerMenu() {
    // http://dojotoolkit.org/reference-guide/1.9/dijit/form/DropDownButton.html#dijit-form-dropdownbutton
    var menu = new dijit.DropDownMenu({ style: "display: none;"});

    var menuItem2 = new dijit.MenuItem({
        label: "Добавить слой из Интернета",
        iconClass:"dijitIconSearch",
        onClick: showAddLayersFromWebUI
    });
    menu.addChild(menuItem2);

    var menuItem3 = new dijit.MenuItem({
        label: "Добавить слой из библиотеки слоев",
        iconClass:"dijitFolderOpened",
        onClick: showAddLayersFromLibUI
    });
    menu.addChild(menuItem3);

    var menuItem4 = new dijit.MenuItem({
        label: "Добавить слой с ГИС сервера",
        iconClass:"dijitFolderOpened",
        onClick: showAddLayersFromAGSUI
    });
...
// Create 'Get link to map' aka 'Save' button
function createGetLinkButton() {
    vslib.log('layout.createGetLinkButton');

    try {
        vsapp.checkForLoadFromSave();
    }
    catch(ex) {
        vslib.log("Ошибка в процессе загрузки карты, сообщите разработчику. \n" +
        'createGetLinkButton.checkForLoadFromSave error: ' + ex.description + "\n" + ex.message,
        true);
    }

    var btn = dijit.byId('getLinkButton');
    if(btn) {
        vslib.log('layout.createGetLinkButton. button exists already, skip creation');
        return;
    }
    ...
    btn = new dijit.form.Button({
        label: "Ссылка",
        id: 'getLinkButton',
        title: "Получить ссылку на карту",
        iconClass: 'esriLinkIcon',
        onClick: function() {
            try {
                vslib.log('getLinkButton click');
                vsapp.getLink2Map();
                vslib.log('getLinkButton click done');
            }
            catch(ex) {
                vslib.log("Ошибка в процессе сохранения карты, сообщите разработчику. \n" +
                'getLinkButton.click error: ' + ex.description + "\n" + ex.message,
                true);
            }
        }
    });
    dojo.byId('webmap-toolbar-left').appendChild(btn.domNode);
} // createGetLinkButton


// Open 'add layer from web' addLayer dialog
function showAddLayersFromWebUI() {
    // http://dojotoolkit.org/reference-guide/1.8/dijit/Dialog.html
    addLayersFromWebUI.show();
//    dijit.registry.byId("addLayersFromWebUI").show();
}


// Process user input for AddLayersFromWebUI dialog (add layer from url)
function procAddLayersFromWebUI(formdata) {
    // define layer type
    ...
    var dataJson = '';
    try {
        dataJson = dojo.json.stringify(formdata);
        log('procAddLayersFromWebUI: \n' + dataJson); // procAddLayersFromWebUI: {"selectALUIType":"agsWS","textALUIURL":"http://<MyServerName>/ArcGIS/rest/services/<MyServiceName>"}
        if ("" == formdata.textALUIURL) {
            return false;
        }

        if (formdata.textALUIURL.indexOf("FeatureServer") != -1) {
            vslib.log("Добавление слоев типа 'FeatureServer' не поддерживается данной версией программы.\nПопробуйте заменить <FeatureServer> на <MapServer>.", true);
            return false;
        }

        if(formdata.selectALUIType == 'agsWS') {
            vsapp.addAGSLayer(formdata.textALUIURL.trim());
        } else {
            vslib.log("Неизвестный тип слоя: " + formdata.selectALUIType, true);
        }

        addLayersFromWebUI.hide();
    } catch(ex) {
        vslib.dir(ex);
        vslib.log("Ошибка в процессе добавления слоя, проверьте параметры. \n" +
            dataJson + ". \n" +
            'procAddLayersFromWebUI error: ' + ex.description + "\n" + ex.message,
            true);
    }
} // function procAddLayersFromWebUI()


// Open layers library TV
function showAddLayersFromLibUI() {
    //var menuItem3 = new dijit.MenuItem({ label: "Добавить слой из библиотеки слоев", iconClass:"dijitFolderOpened", onClick: showAddLayersFromLibUI });
    try {
        vslib.log('showAddLayersFromLibUI: open layers library TV ...');
        //~ vslib.dir(vs.dijit.tvlib);
        vsapp.openTVLib();
    } catch(ex) {
        vslib.dir(ex);
        vslib.log("Ошибка в процессе открытия библиотеки, обратитесь к разработчику. \n" +
            'showAddLayersFromLibUI error: ' + ex.description + "\n" + ex.message,
            true);
    }
}


// open AGS directory pane
function showAddLayersFromAGSUI() {
    try {
        log('showAddLayersFromAGSUI, open AGS directory pane');
        vsapp.openAGSDir();
    } catch(ex) {
        vslib.log("Ошибка в процессе открытия каталога, обратитесь к разработчику. \n" +
            'showAddLayersFromAGSUI error: ' + ex.description + "\n" + ex.message,
            true);
        vslib.dir(ex);
        vslib.log("error stack: " + ex.stack);
    }
}
...
function executeIdentifyTask(evt) {
    try {
        ...
        var tasksLayerIds = dojo.map(layers, function(layer) {
            ...
            vslib.log("executeIdentifyTask. layer.visibleLayers: ");
            vslib.dir(layer.visibleLayers);
            for (var i = 0; i < layer.visibleLayers.length; i++) {
                var subLayer = layer.visibleLayers[i];
                var li = layer.layerInfos[subLayer];
                vslib.log("executeIdentifyTask. layer.layerInfos[subLayer]: ");
                vslib.dir(li);
                if( (subLayer != -1 && !vslib.isGroupSublayer(li)) &&
                    (li.maxScale == 0 || li.maxScale < scale) &&
                    (li.minScale == 0 || li.minScale > scale)
                ) { visibleLayersInScale.push(subLayer); }
            }
            return visibleLayersInScale;
        });

        layers = dojo.filter(layers, function(layer, ind) {
                return tasksLayerIds[ind].length != 0;
        });
        ...
        dlTasks.then(showResults,
            function(err) {
                vslib.log("executeIdentifyTask.taskResponce. error: ");
                vslib.dir(err);
            }
        ...
        for (var i = 0; i < tasks.length; i++) { //Use 'for' instead of 'for...in' so you can sync tasks with defTasks
            try {
                ...
                identifyBusy = false;
                vslib.log("executeIdentifyTask.forEachTask. Error caught: ");
                vslib.dir(lex);
                defTasks[i].errback(lex); //If you get an error for any task, execute the errback
            }
        }

        if (tasks.length == 0) {
            identifyBusy = false;
            map.infoWindow.setContent("Нет слоев с данными.");
            map.infoWindow.show(evt.mapPoint);
        }
    }
    catch(ex) {
        identifyBusy = false;
        vslib.log("Ошибка в процессе обработки запросов к серверам. \n" +
            'executeIdentifyTask error: ' + ex.description + "\n" + ex.message,
            true);
        this.log('executeIdentifyTask error stack: ' + ex.stack);
        window.lastex = ex;
    }
} // function executeIdentifyTask(evt)
...
function showResults(r) {
    var results = [];
    try {
        vslib.log("showResults. responce: ");
        vslib.dir(r);
        //~ identifyBusy = false;
        //~ return results;

        r = dojo.filter(r, function (rr) {
            return rr[0];
        ...
        if(results.length === 0) {
            map.infoWindow.clearFeatures();
            map.infoWindow.setContent("Нет данных.");
        } else {
        ...
        identifyBusy = false;
        vslib.log("Ошибка в процессе обработки ответа от сервера. \n" +
            'showResults error: ' + ex.description + "\n" + ex.message,
            true);
        this.log('showResults error stack: ' + ex.stack);
        window.lastex = ex;
    }
    identifyBusy = false;
    return results;
