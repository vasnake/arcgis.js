try {
    console.log('vs.mpvlib console check');
} catch(ex) {
    var cn = new Object();
    cn.log = function(v){ window.status='log: '+v+''; };
    cn.dir = function(v){ window.status='obj: '+v+''; };
    cn.debug = cn.log; cn.table = cn.dir;
    window.console = cn;
    console = cn;
} // console check

console.log('vs.mpvlib enter');
...
log: function(str, doalert) { // write app messages to log
    try {
        ...
        if(arguments.length > 2) console.log(this.getTimeStamp(), arguments);
...
isBasemapLayer: function(map, lyr) {
    try {
        //this.log('vs.mpvlib.isBasemapLayer. layer: ' + this.toJson(lyr));
        this.log("mpvlib.isBasemapLayer. map, lyr: ", arguments);
        ...
        if(lyr.loaded) {
            ...
            // this check is incorrect sometime. If map not ready yet
            console.debug("vs.mpvlib.isBasemapLayer. map.layerIds, lyr.id", this.toJson({1: map.layerIds, 2: lyr.id}));
            var firstLyrID = map.layerIds[0];
            if(firstLyrID == lyr.id) {
                this.log('vs.mpvlib.isBasemapLayer. true: map.layers[0] == lyr');
                return true;
            }
        }
    } catch(ex) {
        console.log("Error in isBasemapLayer. \n" + 'message: ' + ex.description + "\n" + ex.message, ex);
